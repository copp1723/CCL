...
async function checkEnvVariables(): Promise<boolean> {
  ...
  const missingVars = requiredEnvVars.filter(
    varName => !(appConfig as Record<string, any>)[varName]
  );

  if (missingVars.length > 0) {
    printMessage(
      ERROR_PREFIX,
      `Missing required environment variables: ${chalk.red(missingVars.join(", "))}`
    );
    printMessage(
      INFO_PREFIX,
...
async function testDatabaseConnection(): Promise<boolean> {
  ...
  try {
    ...
    printMessage(
      SUCCESS_PREFIX,
      `Database connection successful to: ${chalk.green(config.get().DATABASE_URL?.split("@")[1] || "DB HOST")}`
    );
    return true;
  } catch (error) {
...
async function runDatabaseMigrations(): Promise<boolean> {
  ...
  try {
    ...
    if (!fs.existsSync(drizzleConfigPath)) {
      printMessage(
        WARN_PREFIX,
        "drizzle.config.ts not found. Skipping migrations. Ensure your DB schema is up-to-date manually."
      );
    ...
    const { stdout, stderr } = await execa("npm", ["run", "db:migrate"], {
      cwd: projectRoot,
      reject: false,
    });
    ...
    if (
      stderr &&
      !stderr.includes("No migrations to apply") &&
      !stderr.includes("All migrations already applied")
    ) {
      ...
      const relevantStderr = stderr
        ...
        .filter(
          line =>
            !line.includes("drizzle-kit:migration") &&
            !line.includes("drizzle-kit:migrator") &&
            line.trim() !== ""
        )
        .join("\n");
      if (relevantStderr) {
    ...
    if (
      stdout.includes("No migrations to apply") ||
      stdout.includes("All migrations already applied") ||
      (stderr &&
        (stderr.includes("No migrations to apply") ||
          stderr.includes("All migrations already applied")))
    ) {
...
async function seedTestData(): Promise<boolean> {
  ...
  try {
    ...
    const { stdout, stderr } = await execa("npm", ["run", "seed", "--", "--full"], {
      cwd: projectRoot,
    }); // Added --full to ensure all data
...
async function validateServices(): Promise<boolean> {
  ...
  if (appConfig.MAILGUN_API_KEY && appConfig.MAILGUN_DOMAIN) {
    try {
      ...
      if (mailgunStatus.success) {
        printMessage(
          SUCCESS_PREFIX,
          `Mailgun connection successful for domain: ${chalk.green(mailgunStatus.domain)}`
        );
      } else {
        printMessage(
          ERROR_PREFIX,
          `Mailgun connection failed for domain ${mailgunStatus.domain}: ${chalk.red(mailgunStatus.error || "Unknown error")}`
        );
    ...
    printMessage(
      WARN_PREFIX,
      "Mailgun API Key or Domain not configured. Skipping Mailgun validation."
    );
  ...
  if (appConfig.OPENROUTER_API_KEY) {
    ...
    printMessage(
      WARN_PREFIX,
      "OpenRouter API Key not configured. AI agent responses will be limited."
    );
    // Not necessarily a failure for setup, but a warning.
  }
...
function printNextSteps() {
  ...
  console.log(
    `     If a client/dashboard is part of this project, start it (e.g., ${chalk.yellow("npm run --filter client dev")}).`
  );
  console.log(
    `     Open your browser to ${chalk.yellow(config.get().FRONTEND_URL || "http://localhost:5173")}.`
  );
  console.log("");
  console.log(`  3. ${chalk.bold("Begin Testing Conversation Workflow")}:`);
  console.log(
    `     - Use the seeded leads (e.g., from ${chalk.yellow("test_leads.csv")}) to initiate conversations.`
  );
  console.log(
    `     - Send emails to your configured Mailgun receiving address to simulate lead replies.`
  );
  ...
  printMessage(
    INFO_PREFIX,
    `Refer to ${chalk.yellow("REAL_DATA_TESTING_GUIDE.md")} for detailed testing scenarios.`
  );
  console.log("");
}
...
async function setup() {
  ...
  if (!(await validateServices())) {
    printMessage(
      WARN_PREFIX,
      "Service validation finished with warnings/errors. Proceed with caution."
    );
