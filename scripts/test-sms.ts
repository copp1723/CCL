  ...
  if (testPhoneNumber) {
    ...
    try {
      const result = await twilioSms.sendSms({
        to: testPhoneNumber,
        message: 'This is a test message from CCL MVP Automation Pipeline. If you received this, the SMS service is working correctly!',
        priority: 'normal'
      });
  ...
  for (let step = 1; step <= 3; step++) {
    ...
    if (testPhoneNumber && twilioSms.getServiceInfo().configured) {
      try {
        ...
        console.log(`   ❌ Error sending recovery message: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    } else {
