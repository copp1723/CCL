...
class SystemValidator {
  ...
  async runAllTests() {
    ...
    await this.test('API Import Endpoint', async () => {
      ...
      const response = await axios.post(`${BASE_URL}/api/data-ingestion/leads/api-import`, testData, {
        headers: { 'x-api-key': API_KEY }
      });
    ...
    await this.test('Email Capture Endpoint', async () => {
      ...
      const response = await axios.post(`${BASE_URL}/api/data-ingestion/leads/email-capture`, emailData, {
        headers: { 'x-api-key': API_KEY }
      });
