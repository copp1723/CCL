...
export class TwilioSmsService {
  ...
  private initialize(): void {
    ...
    if (messagingConfig.twilio.configured) {
      try {
        this.client = twilio(
          messagingConfig.twilio.accountSid!,
          messagingConfig.twilio.authToken!
        );
  ...
  async sendSms(smsMessage: SmsMessage): Promise<SmsResult> {
    ...
    try {
      ...
      if (smsMessage.visitorId) {
        await this.recordOutreachAttempt(smsMessage, undefined, 'failed', error instanceof Error ? error.message : 'Unknown error');
      }
  ...
  async handleDeliveryStatus(twilioWebhook: any): Promise<void> {
    try {
      ...
      logError(error as Error, { webhook: twilioWebhook }, 'Failed to handle Twilio delivery status');
    }
  }
  ...
  private async recordOutreachAttempt(
    ...
    try {
      ...
      outreachLogger.error({ error, visitorId: smsMessage.visitorId }, 'Failed to record outreach attempt');
    }
  }
