...
export class SftpIngestorService {
  ...
  async ingestDailyFiles(): Promise<SftpIngestResult> {
    ...
    try {
      ...
      const csvFiles = remoteFiles.filter(file => 
      ...
      sftpLogger.info({ processingTime: Date.now() - startTime }, 'SFTP ingestion process completed');
    }
  }

  private async processFile(fileName: string, fileSize: number): Promise<{
    success: boolean;
    rowCount: number;
    errorCount: number;
    processingTime: number;
  ...
  private async parseCsvFile(filePath: string, fileName: string): Promise<{
    rowCount: number;
    errorCount: number;
  }> {
  ...
  private async processVisitorRow(row: any, source: string): Promise<void> {
    try {
      ...
      throw new Error(`Failed to process visitor row: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async ingestSingleFile(filePath: string, source: string = 'manual'): Promise<{
    success: boolean;
    rowCount: number;
    errorCount: number;
    processingTime: number;
    ...
    try {
      ...
      const processingTime = Date.now() - startTime;
      logError(error as Error, { filePath, source, processingTime }, 'Manual file ingestion failed');
