...
function verifyTwilioSignature(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  try {
    ...
    outreachLogger.debug({ signature: twilioSignature.substring(0, 10) + '***' }, 'Twilio webhook signature received');
...
router.post('/twilio/status', express.urlencoded({ extended: true }), verifyTwilioSignature, async (req, res) => {
  try {
    outreachLogger.info({ 
...
router.post('/twilio/reply', express.urlencoded({ extended: true }), verifyTwilioSignature, async (req, res) => {
  try {
    ...
    const responseMessage = "Thanks for your reply! Please continue your application at the link we sent you, or call us at (555) 123-4567 to speak with Cathy directly.";
